...
|      promote-self      | string |  false   | `"false"` | Set to "true" to also promote <br>the CALLER repo's own <version> release <br>(unset pre-release, set Latest). Required when the caller publishes <br>stable cuts with the GitHub "None" <br>label, since nothing else ever flips <br>them. Latest is gated by the <br>same backport check as :latest. A <br>failure to set Latest is fatal <br>and aborts before the oss-repo and <br>Homebrew promotions, because that pointer is <br>this action's own backport baseline and <br>a stale one would let a <br>later promotion move :latest backwards; re-run <br>the action to recover.  |
...
Unlike every other release edit here, a failure of the edit that carries
`--latest` is **fatal**, and it aborts before the `oss-repo` and Homebrew
promotions. That one pointer is the action's own backport baseline: the moving
docker tags have already advanced to `version` by this point, so if Latest does
not follow, the next run reads a stale baseline and a later older-line promotion
passes the unscoped gate and moves `:latest` backwards. A warning on a green run
is not enough to prevent that. A failure of a backport edit (`--prerelease=false`
only, moving no pointer) stays a warning. The action is idempotent, so the
recovery is simply to re-run it.
