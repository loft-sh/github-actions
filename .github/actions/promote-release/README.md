...
|      promote-self      | string |  false   | `"false"` | Set to "true" to also promote <br>the CALLER repo's own <version> release <br>(unset pre-release, set Latest). Required when the caller publishes <br>stable cuts with the GitHub "None" <br>label, since nothing else ever flips <br>them. Latest is gated by the <br>same backport check as :latest. On <br>an advancing promotion this edit runs <br>BEFORE the docker retags and any <br>failure to complete it is fatal, <br>because that pointer is this action's <br>own backport baseline: retagging first would <br>leave it behind :latest and let <br>a later promotion move :latest backwards. <br>Re-run the action to recover. On <br>a backport the edit moves no <br>pointer, so a failure stays a <br>warning.  |
