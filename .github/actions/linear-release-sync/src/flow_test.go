...
func TestLinearPullRequestIssueExtraction(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:           "Issue only in branch name",
			prBody:         "Update documentation",
			prBranch:       "ENG-9012/update-docs",
			expectedIssues: []string{},
			description:    "Should ignore issue IDs from branch names",
		},
		{
			name:           "Different issues in body and branch",
			prBody:         "Fix bug ENG-1111",
			prBranch:       "ENG-2222/fix-implementation",
			expectedIssues: []string{"eng-1111"},
			description:    "Should extract only the issue ID from the PR body",
		},
		{
