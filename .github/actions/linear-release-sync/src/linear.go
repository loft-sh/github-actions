...
func NewLinearClient(ctx context.Context, token string, logger *slog.Logger) LinearClient {
...
func (l *LinearClient) ListTeams(ctx context.Context) ([]AvailableTeam, error) {
	...
	for {
		...
		for _, t := range query.Teams.Nodes {
			teams = append(teams, AvailableTeam{Name: t.Name, Key: t.Key})
		}

		if !query.Teams.PageInfo.HasNextPage {
			break
		}
		endCursor := graphql.String(query.Teams.PageInfo.EndCursor)
		cursor = &endCursor
	}
...
// IssueDetails contains state, team, and project information for an issue
...
// GetIssueDetails returns state, team, and project information for an issue.
func (l *LinearClient) GetIssueDetails(ctx context.Context, issueID string) (*IssueDetails, error) {
	...
	return &IssueDetails{
		...
		ProjectName: query.Issue.Project.Name,
	}, nil
}
...
func (l *LinearClient) updateIssueState(ctx context.Context, issueID, releasedStateID string) error {
	...
	if err := l.client.Mutate(ctx, &mutation, variables); err != nil {
		return fmt.Errorf("mutation failed: %w", err)
	}
	if !mutation.IssueUpdate.Success {
		return fmt.Errorf("mutation failed: issue update returned success=false for %s", issueID)
	}
...
func (l *LinearClient) ListIssueComments(ctx context.Context, issueID string) ([]string, error) {
	...
	for {
		...
		endCursor := graphql.String(query.Issue.Comments.PageInfo.EndCursor)
		cursor = &endCursor
	}
...
func (l *LinearClient) createComment(ctx context.Context, issueID, releaseComment string) error {
	...
	if !mutation.CommentCreate.Success {
		return fmt.Errorf("mutation failed: comment create returned success=false for %s", issueID)
	}
