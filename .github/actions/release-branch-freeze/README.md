...
| extra-bypass-team-ids | string |  false   |            | Optional comma-separated numeric team ids added <br>to the freeze bypass alongside bypass-team-id <br>(e.g. a release-automation bot team so <br>loft-bot can merge during a freeze). <br>Individual users cannot be ruleset bypass <br>actors, so a bot must bypass <br>via a team it belongs to.  |
