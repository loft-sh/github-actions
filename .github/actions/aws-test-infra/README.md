...
| primary-private-ip  | string |     Private IP of the role labeled <br>"primary" (empty if not present). Unlike primary-public-ip (positional: first instance), this <br>agrees with primary-instance-id and private-ip-by-role.       |
|  primary-public-ip  | string |                                                                                   Public IP of the primary instance                                                                                    |
| private-ip-by-role  | string |                                          JSON map of role → private <br>IP. Consumer accesses with `fromJSON(steps.<id>.outputs.private-ip-by-role).<role>`.                                           |
