...
|  is_prerelease   | string |  false   |          `"false"`           |                                                                                                                                                                            Is this a pre-release? When "true" <br>the banner gains a "GitHub label: <br>Pre-release" field. Left at the default <br>no label field is rendered at <br>all, so a caller that does <br>not set it sees the same <br>banner as before this field existed.                                                                                                                                                                              |
| needs_promotion  | string |  false   |          `"false"`           | Set to "true" when the release <br>is published un-promoted and a human <br>still has to promote it (the GitHub "None" label: not a pre-release, not Latest). <br>Adds a "GitHub label" field to <br>the banner pointing at the promote <br>workflow, since the release build is <br>the only thing that tells the <br>Release Captain a step is left. <br>Opt-in: with neither this nor is_prerelease <br>set, no label field is rendered <br>and the banner is byte-identical to <br>the one callers got before these <br>inputs existed, so products whose releases <br>ship straight to Latest are unaffected.  |
