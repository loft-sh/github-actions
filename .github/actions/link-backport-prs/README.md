...
4. Finds the modern `backport/<target>/pr-<source>` PR in the source repo and split backports in the configured pro and OSS repos. New split PRs use `backport/<target>/<full-source-merge-sha>`; a cross-repository match requires the exact source merge SHA, expected head repository, and target base, so public body edits cannot affect discovery. Source-repository PRs still require the fully-qualified source reference. Older `backport/<target>/<sha-prefix>` PRs remain compatible through that reference, and the transitional public source marker remains accepted. The producer-owned branch format is documented in the [`backport-legacy-split` README](../backport-legacy-split/README.md#pr-title-and-body).
