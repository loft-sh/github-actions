...
4. Finds the modern `backport/<target>/pr-<source>` PR in the source repo and any legacy `backport/<target>/<short-sha>` PRs in the configured pro and OSS repos. A legacy match requires the expected head repository, base branch, and merge-SHA branch prefix. It also requires either the fully-qualified source PR reference used by private and older PRs, or the full-SHA `legacy-backport-source` marker emitted on new public OSS PRs. The marker must remain when editing a public PR body. The old reference remains accepted so existing backports can finish without migration. The producer-owned marker format is documented in the [`backport-legacy-split` README](../backport-legacy-split/README.md#pr-title-and-body).
