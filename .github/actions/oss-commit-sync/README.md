...
|      loose-absorption      | string | Export: true when an external counts <br>as absorbed only via an Oss-Commit <br>line outside the block git's own <br>trailer parser reads AND its content <br>is not in the subtree, i.e. <br>the one combination align-tree refuses to <br>overwrite. The export itself proceeds. Do <br>NOT dispatch the import direction on <br>this alone, it has nothing to <br>replay. When diverged is also true, <br>diverged wins: dispatch the import for <br>the unabsorbed commits. Use the health <br>direction to find squash-orphaned trailers generally.  |
