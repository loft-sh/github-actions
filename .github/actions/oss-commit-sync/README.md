...
|      loose-absorption      | string | Export: true when an external counts <br>as absorbed only via an Oss-Commit <br>line outside the block git's own <br>trailer parser reads AND its content <br>is not in the subtree, i.e. <br>the one combination align-tree refuses to <br>overwrite. An ordinary export still proceeds; <br>a run with align-tree true fails <br>instead, since that overwrite is what <br>is being refused. Do NOT dispatch <br>the import direction on this alone, <br>it has nothing to replay. When <br>diverged is also true, diverged wins: <br>dispatch the import for the unabsorbed <br>commits. Use the health direction to <br>find squash-orphaned trailers generally.  |
